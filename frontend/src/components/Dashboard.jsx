...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (storyEvent && storyEvent.description) {
      let i = 0;
      setTypedStory("");
      const interval = setInterval(() => {
        setTypedStory((prev) => prev + storyEvent.description.charAt(i));
        i++;
        if (i >= storyEvent.description.length) {
    ...
    <div className={styles.dashboard}>
      ...
      <div className={styles.mainContent}>
        ...
        <section className={styles.aiEvent}>
          <h2>
            📜 AI Story Event{" "}
            <span className={styles.aiBadge}>🤖 LLM Generated</span>
          </h2>
          {storyEvent ? (
            <>
              <p className={styles.animatedStory}>{typedStory}</p>
              <p>
              ...
              <p className={styles.generatedBy}>
                🤖 Generated by: Narrative Agent (LLM)
              </p>
            </>
          ) : (
        ...
        <section className={styles.riskAnalysis}>
          <h2>
            ⚠️ AI Risk Analysis{" "}
            <span className={styles.aiBadge}>🧠 ML Model</span>
          </h2>
          {riskAssessment ? (
            <>
              <p>
          ...
          <div className={styles.npcList}>
            {worldState.npcs &&
              Object.values(worldState.npcs).map((npc) => {
                const decision = npc.decision || npc.last_action || "idle";
                let reason = npc.reason;
                if (!reason) {
                  if (worldState.danger_level > 70) {
                    reason = "High danger triggered defensive behavior";
                  } else if (npc.health < 30) {
                    reason = "Low health triggered retreat behavior";
                  } else if (npc.mood_level > 60) {
                    reason = "Friendly mood triggered assist behavior";
                  } else {
                    reason = "Default patrol behavior";
                  }
                }
                // simple color logic for decision badge
                const decisionColor =
                  decision.toLowerCase().includes("defend") ||
                  decision.toLowerCase().includes("attack")
                    ? "#f44336"
                    : decision.toLowerCase().includes("retreat")
                      ? "#ff9800"
                      : "#4caf50";
                ...
                return (
                  <div key={npc.id} className={styles.npcCard}>
                    <h3>
                      {npc.name}{" "}
                      <span className={styles.aiBadge}>⚙️ Behavior Tree</span>
                    </h3>
              ...
              worldState.events
                ...
                .map((event, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className={`${styles.timelineItem} ${isNew ? styles.new : ""}`}
                    >
                      <div className={styles.marker}></div>
                      <div className={styles.eventContent}>
                        <span className={styles.timeIcon}>🕒</span> {event}
                      </div>
                    </div>
                  );
                })}
